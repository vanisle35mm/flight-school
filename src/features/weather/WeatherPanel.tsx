...
const runwayEndPoint = (heading: number, distance: number) => runwayPoint((heading + 180) % 360, distance);
  ...
  return <div className="runway-layout-briefing">
    <div className="runway-airport-map">
      <svg viewBox="0 0 420 270" role="img" aria-label="Airport runway layout with wind-favoured runway end highlighted">
        ...
        <g className="runway-map-compass" aria-hidden="true">
          <circle cx="210" cy="135" r="118" />
          <text x="210" y="30">N</text>
        </g>
        ...
        {runways.map((runway) => {
          ...
          const start = runwayEndPoint(runway.ends[0].heading, halfLength);
          const end = runwayEndPoint(runway.ends[1].heading, halfLength);
          const firstLabel = runwayEndPoint(runway.ends[0].heading, halfLength + 19);
          const secondLabel = runwayEndPoint(runway.ends[1].heading, halfLength + 19);
          const preferred = runway.preferredEnd ? runway.ends.find((candidate) => candidate.id === runway.preferredEnd) : null;
          const preferredPoint = preferred ? runwayEndPoint(preferred.heading, halfLength - 10) : null;
    ...
    <div className="runway-layout-summary">
      <span>North-up runway layout</span>
      <strong>{bestRunway?.preferredEnd ? `RWY ${bestRunway.preferredEnd}` : 'Runway unavailable'}</strong>
      <small>{bestRunway?.preferredEnd ? `Best wind alignment on ${bestRunway.id}` : 'Runway wind estimate needs wind and runway data'}</small>
    </div>
  </div>;
};
  ...
  return <section className="panel weather-panel weather-panel-full">
    ...
    <div className="weather-briefing-layout">
      {runwayWinds.length > 0 && <section className="runway-weather-section">
        ...
        <div className="runway-list runway-list-compact">
          {runwayWinds.map((runway) => {
            ...
            return <div className={isBest ? 'runway-row best-runway' : 'runway-row'} key={runway.id}>
              <div className="runway-identity">
                ...
                <span>{runway.lengthFt.toLocaleString()} ft / {runway.surface}</span>
              </div>
              <div className="runway-components">
                <div><span>Use</span><strong>{runway.preferredEnd ? `RWY ${runway.preferredEnd}` : '--'}</strong></div>
                <div><span>Head</span><strong>{runway.headwindKt === null ? '--' : `${runway.headwindKt} kt`}</strong></div>
                <div><span>Cross</span><strong>{runway.crosswindKt === null ? '--' : `${runway.crosswindKt} kt`}</strong></div>
              </div>
              {isBest && <span className="best-runway-label"><PlaneLanding size={14} />Favoured</span>}
            </div>;
          })}
        </div>
      ...
      <aside className="weather-briefing-side">
        <section className="weather-condition-block">
          <span className={getCategoryClass(snapshot.flightCategory)}>{snapshot.flightCategory}</span>
        ...
        <section className="wind-instrument">
        ...
        <section className="aviation-report-band">
          <div className="weather-section-heading"><div><Compass size={20} /><h3>METAR</h3></div><span>{snapshot.observed}</span></div>
          <p className="raw-weather-report">{snapshot.metar}</p>
        </section>
      </aside>
    </div>

    {snapshot.forecast.length > 0 && <section className="weather-detail-section weather-forecast-strip">
