import { Bell, BookOpen, CheckSquare, ChevronDown, CloudSun, Gauge, GraduationCap, Layers, LogOut, Plane, Search, Settings, ShieldCheck, SlidersHorizontal, UserRound } from 'lucide-react';
...
const titleForView = (view: ViewId) => view === 'import' ? 'Import' : view === 'dashboardEdit' ? 'Edit Dashboard' : view === 'users' ? 'Admin Console' : navItems.find((item) => item.id === view)?.label ?? 'Dashboard';
...
export const Shell = ({ children, activeView, onViewChange, search, onSearchChange, activeUserName, canAdmin, cloudStatus, onLogout }: { children: ReactNode; activeView: ViewId; onViewChange: (view: ViewId) => void; search: string; onSearchChange: (value: string) => void; activeUserName: string; canAdmin: boolean; cloudStatus: CloudSyncStatus; onLogout: () => void }) => <div className="app-shell cockpit-shell">
  ...
  <main className="main-area cockpit-main">
    <header className="topbar cockpit-topbar">
      ...
      <div className="status-cluster" aria-label="Flight status"><button className="user-pill" onClick={() => canAdmin ? onViewChange('users') : onLogout()}><UserRound size={16} />Captain {activeUserName}</button><span className={`cloud-sync-pill ${cloudStatus}`}><CloudSun size={17} />{cloudStatusLabel[cloudStatus]}</span><span className="station-pill">CYYJ <ChevronDown size={14} /></span><span className="weather-pill"><CloudSun size={17} />17 C</span><button className="bell-button" aria-label="Notifications"><Bell size={18} /></button></div>
    </header>
    {children}
  </main>
</div>;
