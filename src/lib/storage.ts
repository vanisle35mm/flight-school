...
export const normalizeGroundSchoolData = (value: unknown): GroundSchoolData => {
  ...
  Object.entries(sourceUsers).forEach(([id, value]) => {
    ...
    users[safeId] = {
      ...
      ...(typeof item.introGuideSeen === 'boolean' ? { introGuideSeen: item.introGuideSeen } : {}),
  ...
  if (!Object.keys(users).length) {
    users[DEFAULT_USER_ID] = {
      ...
      introGuideSeen: typeof source.introGuideSeen === 'boolean' ? source.introGuideSeen : false,
      tcHistory: [],
      tcMissedIds: [],
      rocaHistory: [],
      rocaMissedIds: [],
